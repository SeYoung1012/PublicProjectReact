...
function EgovBatchSchdulRegist()  {
  ...
  const formSubmit = async()=>{
    ...
    if(registForm.executCycle=="02" && Array.from(checkedDays).length<1){
      ...
      try{
        const resp = await axios.post(SERVER_URL+'/sym/bat/addBatchSchdul.do',{
          ...registForm,
          batchOpertId:batchOpertIdRef.current.value,
          batchOpertNm:batchOpertNmRef.current.value,
          executSchdulDfkSes:Array.from(checkedDays),
      ...
      <form>
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader>
              <strong>배치스케줄 등록</strong> <small></small>
            </CCardHeader>
            <CCardBody>
...
export default EgovBatchSchdulRegist;