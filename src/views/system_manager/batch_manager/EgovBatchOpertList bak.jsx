  ...
  const retrieveList = async(searchVO)=>{
    try{
      ...
      resp.data.result.resultList.forEach(function (item, index) {
        ...
        mutResultList.push(
          <CTableRow onClick={()=>navigate('/system_manager/batch_manager/EgovBatchOpertDetail',{state:{batchOpertId:item.batchOpertId}})}>
    ...
    <CRow>
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>배치작업관리 목록</strong> <small></small>
          </CCardHeader>
          <CCardBody>
          <CForm className="row g-3 d-flex justify-content-end">
            ...
            <CFormInput  type="button"
                    value="등록"
                    onClick={()=>navigate('/system_manager/batch_manager/EgovBatchOpertRegist')}/>
            </CCol>
            </CForm>
