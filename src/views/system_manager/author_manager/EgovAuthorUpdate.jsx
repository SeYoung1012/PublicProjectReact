...
function EgovAuthorUpdate() {
  ...
  const formSubmit=async(registForm)=>{
    try{
      const resp = await axios.post(SERVER_URL+'/sec/ram/EgovAuthorUpdateAPI.do',registForm)
      if(resp.data.resultCode == CODE.RCV_SUCCESS){
        alert(resp.data.resultMessage);
        navigate('/system_manager/author_manager/EgovAuthorUpdate',{state:{authorCode:registForm.authorCode}})
      }else{
        alert(resp.data.resultMessage);
      }
    }catch(error){
