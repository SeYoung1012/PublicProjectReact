...
function EgovLoginLogList () {
    ...
    const retrieveList = (searchVO) => {
      axios
        .get(SERVER_URL+'/sym/log/clg/SelectLoginLogListAPI.do', {
          params: searchVO,
        })
        .then(
          (resp) => {
            ...
            resp.data.result.resultList.forEach(function (item, index) {
              ...
              let creatDt = item.creatDt.substring(0, 19)
              console.log(creatDt)
              mutLogList.push(
                <CTableRow>
                  <CTableHeaderCell scope="row">{listIdx}</CTableHeaderCell>
                  <CTableDataCell>{item.logId}</CTableDataCell>
                  <CTableDataCell>{item.creatDt}</CTableDataCell>
                  <CTableDataCell>{item.loginMthd}</CTableDataCell>
                  <CTableDataCell>{item.loginNm}</CTableDataCell>
                  <CTableDataCell>{item.loginIp}</CTableDataCell>
                  <CTableDataCell
                    onClick={() =>
                      window.open('http://localhost:3000#/system_manager/log_manager/EgovLoginLogDetail?logId='+
                      item.logId,'_blank','height=800')
                    }
                  >
      ...
      <CRow>
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader>
              <strong>접속 로그 관리</strong> <small></small>
            </CCardHeader>
            <CCardBody>
                ...
                <CTable hover>
                  <CTableHead>
                    <CTableRow>
                      ...
                      <CTableHeaderCell scope="col">로그ID</CTableHeaderCell>
                      <CTableHeaderCell scope="col">발생일자</CTableHeaderCell>
                      <CTableHeaderCell scope="col">접속방식</CTableHeaderCell>
                      <CTableHeaderCell scope="col">사용자명</CTableHeaderCell>
                      <CTableHeaderCell scope="col">접속IP</CTableHeaderCell>
...
export default EgovLoginLogList
