  ...
  const retrieveList = async(searchVO)=>{
    try{
      ...
      resp.data.result.resultList.forEach(function (item, index) {
        ...
        mutResultList.push(
          <CTableRow onClick={()=>navigate('/system_manager/code_manager/EgovCcmCmmnCodeDetail',{state:{codeId:item.codeId}})}>
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          ...
          <CForm className="row g-3 d-flex justify-content-end">
            ...
            <CFormInput  type="button"
                    value="등록"
                    onClick={()=>navigate('/system_manager/code_manager/EgovCcmCmmnCodeRegist')}/>
            </CCol>
            </CForm>
