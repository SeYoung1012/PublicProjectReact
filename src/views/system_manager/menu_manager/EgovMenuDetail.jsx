...
function EgovMenuDetail()  {
  ...
  const callInfo = async() => {
    try{
      const resp = await axios.get(SERVER_URL+'/sym/mnu/mpm/EgovMenuManageListDetailSelectAPI.do',{
        params:{
          searchKeyword:menuNo
        }
      })
      if(resp.data.resultCode == CODE.RCV_ERROR_AUTH){
        alert(resp.data.resultMessage);
      ...
      menuNoRef.current.value = resp.data.result.menuManageVO.menuNo;
      menuOrdrRef.current.value = resp.data.result.menuManageVO.menuOrdr;
      menuNmRef.current.value = resp.data.result.menuManageVO.menuNm;
      upperMenuIdRef.current.value = resp.data.result.menuManageVO.upperMenuId;
      progrmFileNmRef.current.value = resp.data.result.menuManageVO.progrmFileNm;
      relateImageNmRef.current.value = resp.data.result.menuManageVO.relateImageNm;
      relateImagePathRef.current.value = resp.data.result.menuManageVO.relateImagePath;
      menuDcRef.current.value = resp.data.result.menuManageVO.menuDc;
      console.log(menuNoRef.current.value);
    }catch(error){
      console.error(error)
    }
  }
  ...
  const formSubmit = async() =>{
    ...
    if(confirm('수정하시겠습니까?')){
      if(menuNoRef.current.value==null || menuNoRef.current.value=="") alert('메뉴No는 필수입력사항 입니다');
        ...
        try{
          const resp = await axios.post(SERVER_URL+'/sym/mnu/mpm/EgovMenuManageUpdtAPI.do',{
  ...
  const formDelete = async() =>{
    ...
    if(confirm('삭제하시겠습니까?')){
      ...
      try{
        const resp = await axios.post(SERVER_URL+'/sym/mnu/mpm/EgovMenuManageDeleteAPI.do',{
          menuNo:menuNoRef.current.value,
          menuOrdr:menuOrdrRef.current.value,
          menuNm:menuNmRef.current.value,
          upperMenuId:upperMenuIdRef.current.value,
        ...
        if(resp.data.resultCode == CODE.RCV_SUCCESS){
          alert(resp.data.resultMessage);
          navigate('/system_manager/menu_manager/MenuManager');
        }else{
          alert(resp.data.resultMessage);
        }
      }catch(error){
