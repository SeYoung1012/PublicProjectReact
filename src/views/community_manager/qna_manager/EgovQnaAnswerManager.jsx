...
function EgovQnaAnswerManager() {
    ...
    const retrieveList = async (searchCondition) => {
        try {
            const res = await axios.get(SERVER_URL+'/uss/olh/qna/selectQnaAnswerListAPI.do', {
                params: {
            ...
            res.data.result.resultList.forEach(function (item, index) {
                ...
                dataListTag.push(
                    <CTableRow key={listIdx} onClick={() => navigate('/system_manager/community_manager/EgovQnaAnswerManagerDetail', { state: { qaId: item.qaId } })}>
...
export default EgovQnaAnswerManager