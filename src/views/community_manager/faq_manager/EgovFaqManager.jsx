...
function EgovFaqManager() {
  ...
  const retrieveList = async (searchCondition) => {
      try {
          const res = await axios.get(SERVER_URL+'/uss/olh/faq/selectFaqListAPI.do', {
              params: {
          ...
          res.data.result.resultList.forEach(function (item, index) {
              ...
              dataListTag.push(
                  <CTableRow key={listIdx} onClick={() => navigate('/system_manager/community_manager/EgovFaqManagerDetail', { state: { faqId: item.faqId } })}>
                  ...
                  <CCardBody>
                      <CForm className="row g-3 d-flex justify-content-end">
                          ...
                          <CCol md={1}>
                              <CFormInput type="button"
                                  value="등록"
                                  onClick={() => navigate('/system_manager/community_manager/EgovFaqManagerRegist')}
                              />
                          </CCol>
                      </CForm>
                      <DocsExample >
